...
gp.output(21, True)
gp.output(22, True)
###define here where the cameras should take photos, using 1 to turn it on and 0 to turn off
A_arr=[1, 1, 1, 1, 1, 1];
B_arr=[1, 1, 1, 1, 1, 1];
C_arr=[1, 1, 1, 1, 1, 1];
D_arr=[1, 1, 1, 1, 1, 1];

def capture(A,B,C,D,row,stage):
   if A==1:
    ...
    gp.output(11, False)
    gp.output(12, True)
    cmdA = "sudo raspistill -t 10000 -o /var/www/html/cameraA/image_camA_"+str(stage)+"_"+str(row)+"_%s.jpg" %time.strftime("%Y%m%d-%H%M%S")
    os.system(cmdA)
   ...
   if B==1:
    ...
    gp.output(11, False)
    gp.output(12, True)
    cmdB = "sudo raspistill -t 10000 -o /var/www/html/cameraB/image_camB_"+str(stage)+"_"+str(row)+"_%s.jpg" %time.strftime("%Y%m%d-%H%M%S")
    os.system(cmdB)
    ###For four more photos from the same camera
...
# #   
   if C==1:  
    ...
    gp.output(11, True)
    gp.output(12, False)
    cmdC = "sudo raspistill -t 10000 -o /var/www/html/cameraC/image_camC_"+str(stage)+"_"+str(row)+"_%s.jpg" %time.strftime("%Y%m%d-%H%M%S")
    os.system(cmdC)
#     #For four more photos from the same camera
   ...
   if D==1: 
    ...
    gp.output(11, True)
    gp.output(12, False)
    cmdD = "sudo raspistill -t 10000 -o /var/www/html/cameraD/image_camD_"+str(stage)+"_"+str(row)+"_%s.jpg" %time.strftime("%Y%m%d-%H%M%S")
    os.system(cmdD)
    #For four more photos from the same camera
...
for i in range (0,5): 
   ...
   #     %main()
    j=0;
    capture(A_arr[j*5+i],B_arr[j*5+i],C_arr[j*5+i],D_arr[j*5+i],i,j)
    gp.output(7, False)
    gp.output(11, False)
    gp.output(12, True)   
...
# for i in range (0,1):    #i is the number of rows we filled with wellplates (Maximum:5) 
    #if __name__ == "__main__":
    #    main()
#    j=1;
#    capture(A_arr[j*5+i],B_arr[j*5+i],C_arr[j*5+i],D_arr[j*5+i],i,j)   
...
#    gp.output(11, False)
#    gp.output(12, True)   
#    if i == 0: break #No action in the last step. i should be (range-1)
#    print("Waiting")
#    time.sleep(1)  #waiting for 1 seconds
#    print("Ready for the next round")
    #code of moving linear actuator
